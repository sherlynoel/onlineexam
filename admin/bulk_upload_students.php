...
function insert_into_db($details, $conn) {
  ...
  else
  {
    ...
    $mysql = "INSERT INTO tbl_users (user_id, first_name, last_name, gender,  email, sem_sec, usn, department, phone,dob, batch, course, login,role) VALUES ('$student_id', '$fname', '$lname', '$gender', '$email', '$semSection', '$usn', '$department', '$phone','$dob', '$batch', '$ug_pg', '$new_password','$DEFAULT_ROLE')";
  ...
  if ( $xlsx = SimpleXLSX::parse($_FILES['file']['tmp_name']) ) {
    ...
    foreach ($xlsx->rows() as $instance) {
      ...
      if ($i <= 3) {
        ...
        $details= array("fname"=>"", "lname"=>"", "gender"=>"", "email"=>"", "sem"=>"", "sec"=>"", "usn"=>"", "department"=>"",  "phone"=>"","dob"=>"", "batch"=>"", "course"=>"",);
