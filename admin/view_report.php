...
$sql = "SELECT * FROM tbl_assessment_records WHERE exam_id = '$exam_id' AND student_id = '$student_id'";
...
$sql = "SELECT * FROM tbl_report WHERE exam_id = '$exam_id' AND user_id = '$student_id'";
$result = $conn->query($sql);

echo "<div><b><h2><center>".$exam_name.": </b>".$student_name."</h2></div>";
if ($result->num_rows > 0) {
...
<p><b>Question Number: </b>'.$qno.'<br><b>Question: </b>'.nl2br(htmlentities($qs), false).'<br><b>Answer: </b>'.nl2br(htmlentities($ans), false).'</p>';
                                          if( $imgs == NULL){ }else{
