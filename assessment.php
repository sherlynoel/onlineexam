...
$sq = "";
$char = 'a';
for($i = 0; $i < $numTopic - 1; $i++){
	$sq .= "SELECT * FROM (SELECT * FROM tbl_questions WHERE topic = '$topics[$i]' AND exam_id = '$exam_id' LIMIT $top_que[$i]  ) $char UNION ";
	$char++;
}

$sq .= "SELECT * FROM (SELECT * FROM tbl_questions WHERE topic = '$topics[$i]' AND exam_id = '$exam_id' LIMIT $top_que[$i] ) $char ";
...
<div role="tabpanel" class="tab-pane active fade in" id="tab'.$qno.'">
			...
			<p>Topic: '.$topic.'</p>
			<p style="overflow-x:scroll"><span style="font-size: 10pt;white-space:pre"><b>Question No: '.$qno.'<br></b> '.(htmlentities($qs)).'<span></p>
            ';
              if( $imgs == NULL){ }else{
              print '<img src="data:image/jpeg;base64,'.base64_encode($imgs).'"  class="img-responsive" style="top:0px;width:300px;height:300px" alt="error"/>';
...
<a role="tab"  href="#tab'.($qno-1).'" onclick="check(demo'.($qno-1).')"  data-toggle="tab">Prev</a>
<a role="tab" style="float:right" href="#tab'.($qno+1).'" onclick="check(demo'.($qno+1).')"  data-toggle="tab"> Next</a>
...
<p><span style="font-size: 10pt;white-space:pre">Marks: '.(htmlentities($posmarks)).'<span></p>
<p>Topic: '.$topic.'</p>
<p style="overflow-x:scroll"><span style="font-size: 10pt;overflow-x:scroll;white-space:pre"><b>Question No: '.$qno.'<br></b> '.(htmlentities($qs)).'</span></p>
             ';
              if( $imgs == NULL){ }else{
              print '<img src="data:image/jpeg;base64,'.base64_encode($imgs).'" class="img-responsive" alt="error" style="top:0px;width:300px;height:300px"/>';
...
<a role="tab"  href="#tab'.($qno-1).'" onclick="check(demo'.$qno.')"  data-toggle="tab">Prev</a>
<a role="tab" style="float:right" href="#tab'.($qno+1).'" onclick="check(demo'.($qno+1).')"  data-toggle="tab"> Next</a>
...
<p><span style="font-size: 10pt;white-space:pre">Marks: '.(htmlentities($posmarks)).'<span></p>
<p>Topic: '.$topic.'</p>
<p style="overflow-x:scroll"><span style="font-size: 10pt;overflow-x:scroll;white-space:pre"><b>Question No: '.$qno.'<br></b> '.(htmlentities($qs)).'<span></p><br>
';
if( $imgs == NULL){ }else{
print '<img src="data:image/jpeg;base64,'.base64_encode($imgs).'" class="img-responsive" alt="error" style="top:0px;width:300px;height:300px"/>';
...
if($counter==1){
print '<td><a id="demoz'.$counter.'" href="#tab'.$counter.'" role="tab"  data-toggle="tab" onclick="check(demo'.$counter.')"><button style="background-color: #ffcc00" id="demo'.$counter.'" >'.$counter.'</button></a></td>';	
$counter=$counter+1 ;
}
else {
print '<td><a id="demoz'.$counter.'" href="#tab'.$counter.'" role="tab"  data-toggle="tab" onclick="check(demo'.$counter.')"><button id="demo'.$counter.'">'.$counter.'</button></a></td>';	
...
id.setAttribute("style","background-color:#4BB543;color:white;");
}
}
